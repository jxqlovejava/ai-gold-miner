...
5b. **单次模式**：用户说「单次分析」/「不落盘」/「直接输出别写文件」-> `FORCE_SCAN=1 ASSEMBLE_FORCE=1 bash scripts/quick_scan.sh`（全新扫描+骨架重建，不 REUSE 不转贴历史报告）-> 模型直接文本输出完整报告（3 增量板块内嵌；**2.1-2.8 维度明细 1:1 转贴骨架表格，禁止压缩成散文**），**不 Write 报告文件**。细节见 SKILL.md 铁律 7 单次模式。
