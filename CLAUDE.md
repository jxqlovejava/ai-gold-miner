...
4b. **待查结果必补查**：bundle 骨架含「⚠️ 待查事件 · 必须补查」板块（scan 检测到已发布但未同步实际结果的事件）→ 组装/单次输出前**必须**搜索权威源补查实际值（写日历 `update_event_result` + `gold_bias` 持久化），纳入「近期事件结果回顾」+ 主驱动/目标区间；**补查后的新事件时效加权 <24h=1.0（最高，是 7 天前事件的 3.3 倍）**。禁止跳过——残留占位 Write hook（`validate_report_format.py`）会拦截 exit 2；单次模式靠 SKILL.md 铁律 12 自觉执行。
