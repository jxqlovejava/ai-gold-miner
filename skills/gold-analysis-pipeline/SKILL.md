...
12. **待查结果必须补查进正文（2026-08-26 起）** - scan 内置事件同步检测到「已发布但 actual 为空」的事件会输出「⚠️ 待查结果」提醒，骨架含「⚠️ 待查事件 · 已发布未同步 · 必须补查」强制板块（`assemble_report.py` 程序化生成）。**组装/输出报告时若骨架含待查板块，禁止跳过**（事故：2026-08-25 谘商会消费者信心/里奇蒙德/新屋销售/FHFA/CS房价 5 项已发布数据未进正文）：必须①搜索权威来源补查实际值（**英文一手源必须双引擎交叉验证**：`PYTHONPATH=src python3 scripts/dual_engine_verify.py "<查询>" --from <日期>`，引擎A anysearch + 引擎B wigolo 并标注引擎来源，细则见 `references/event-sync.md` §1.7）-> ②写日历持久化（`update_event_result` + `gold_bias`，命令见 `references/event-sync.md` §1.7 第3步）-> ③将实际结果纳入「近期事件结果回顾」+ 主驱动/目标区间/军规提醒，把待查板块替换为实际数据表 -> ④**补查写日历后必须 `FORCE_SCAN=1` 重跑一次 scan**（事故：2026-08-26 补查 4 项已发布数据只进回顾表、未进事件驱动信号权重——scan 是快照式，信号生成发生在补查前，`actual` 为空的待查事件不产生事件信号）。重跑后 `RecentEventSignalGenerator` 才消费 `get_recent_events_with_results()` 将补查结果纳入事件驱动权重；quick_scan.sh 检测到待查会输出 `PENDING_RESCAN|N` 标记行强制提示。重跑后基于新 bundle 组装；若仍有待查板块则重复补查-重跑循环。**补查后的新事件自动按时效加权（<24h=1.0 最高权重，是 7 天前事件的 3.3 倍，见 event-sync.md §1.8）**。程序强制：报告残留「必须补查」占位 -> `validate_report_format.py` Write hook 拦截（exit 2）；单次模式（铁律 7 单次）不落盘、不过 hook，靠本铁律自觉执行。**补查搜索工具 schema（2026-08-27 起）**：内置 `WebSearch` 仅接受 query/allowed_domains/blocked_domains，**禁止传 time_range/from_date/country**——任何额外参数报 `InputValidationError`（UI 显示 `Invalid tool parameters`，事故 2026-08-26 补查两连错）；时间约束用查询文本内嵌日期，或 anysearch CLI `--days`/wigolo `time_range`。WebFetch 的 `prompt` 必填。细则见 `references/event-sync.md` §1.7。
