...
本项目强调**可验证、可追溯**的投资分析。无论是 AI 代理还是人工分析师，输出金价分析时必须遵守：

### 必须完整展示 Agent 博弈

| Agent | 必须包含 |
|-------|----------|
| BullAgent | 立场、仓位%、信心%、逐条论据及评分 |
| BearAgent | 立场、仓位%、信心%、逐条论据及评分 |
| PortfolioManager | 原始决策 + 军规调整后的最终决策 |

### 必须逐项说明 8 维信号

每个维度必须给出：信号数量、方向、核心数值；**0 信号时必须说明原因**（横盘未触发 / API 失败 / 数据缺失）。

### 必须披露 API 失败

NewsAPI、anysearch、Yahoo Finance、DuckDuckGo 等数据源失败时，必须在输出中写明，禁止隐藏。

### 必须标注 Source Truth

所有外部信息标注 `[verified: T0/T1/T2/T3]` 或 `[unverified]`。
